...
export function ClientDashboard() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const mappedRequests = response.data.map((req: any) => ({
        ...
        details: req.details,
        proposalAmount: req.Proposal
          ? `$${req.Proposal.totalAmount}`
          : undefined,
  ...
  const handleAcceptProposal = async (proposalId?: string) => {
    ...
    if (
      !window.confirm(
        "Are you sure you want to accept this proposal? A new project will be created."
      )
    )
      return;
  ...
  const actionRequiredRequest = requests.find(
    (r) => r.status === "action-required"
  );
  const hasActionRequired = !!actionRequiredRequest;

  const pendingCount = requests.filter(
    (r) => r.status === "pending-review" || r.status === "pending"
  ).length;
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        {/* Welcome Section */}
        <div className="mb-8">
          <h1 className="mb-2 text-[#0D1B2A]">
            Welcome, {user?.name || "Business Owner"}!
          </h1>
          <p className="text-[#4A5568]">
            Request services, review proposals, and manage your projects
          </p>
        </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-gradient-to-br from-[#2EC4B6] to-[#26a599] rounded-xl p-8 text-white shadow-lg">
              ...
              <p className="mb-6 text-white/90">
                Start a new service request and receive custom proposals from
                our digital experts
              </p>
              <button
              ...
              <div className="bg-gradient-to-br from-[#FFF4E6] to-[#FFF9F0] border-2 border-[#F39C12] rounded-xl p-6 shadow-md">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h3 className="mb-2 text-[#0D1B2A]">
                      New Proposal Received!
                    </h3>
                    <p className="text-sm text-[#4A5568] mb-4">
                      {actionRequiredRequest.category} -{" "}
                      <span className="font-semibold text-[#0D1B2A]">
                        {actionRequiredRequest.proposalAmount || "Price TBD"}
                      </span>
                    </p>
                    <div className="flex gap-3">
                      <button
                        onClick={() =>
                          handleViewPdf(actionRequiredRequest.pdfPath)
                        }
                      ...
                      <button
                        onClick={() =>
                          handleAcceptProposal(actionRequiredRequest.proposalId)
                        }
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="text-left py-3 px-4 text-sm font-semibold text-[#4A5568]">
                        Service Category
                      </th>
                      <th className="text-left py-3 px-4 text-sm font-semibold text-[#4A5568]">
                        Date Submitted
                  ...
                  <tbody>
                    {requests.length === 0 ? (
                      <tr>
                        <td
                          colSpan={4}
                          className="py-8 text-center text-[#4A5568]"
                        >
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-white rounded-lg border border-gray-200 p-5 shadow-sm">
                ...
                <p className="text-3xl font-semibold text-[#0D1B2A]">
                  {requests.length}
                </p>
              </div>
              <div className="bg-white rounded-lg border border-gray-200 p-5 shadow-sm">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-[#2EC4B6]/10 rounded-lg flex items-center justify-center group-hover:bg-[#2EC4B6] transition-colors">
                      <Plus
                        className="text-[#2EC4B6] group-hover:text-white"
                        size={18}
                      />
                    </div>
