    ...
    <>
      <div className="mb-8 mt-15 md:mt-0">
        ...
        <p className="text-[#4A5568]">
          Track the complete lifecycle of client requests
        </p>
      </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
              ...
              <p className="text-gray-500">
                No request history found for this client.
              </p>
            </div>
          ) : (
            <div className="max-w-4xl mx-auto pb-12">
              {timelineData.map((item) => (
                <div
                  key={item.requestId}
                  className="mb-8 relative pl-8 border-l-2 border-gray-200 last:border-0"
                >
                    ...
                    <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
                      ...
                      <span>
                        ...
                        <span className="font-medium text-[#0D1B2A]">
                          {item.agentName || "Pending Assignment"}
                        </span>
                      </span>
                    </div>
                      ...
                      >
                        <div className="flex items-center gap-2 mb-2">
                          <FileCheck
                            size={18}
                            className={
                              item.proposal ? "text-blue-600" : "text-gray-400"
                            }
                          />
                          <span
                            className={`font-semibold ${
                              item.proposal ? "text-blue-900" : "text-gray-400"
                            }`}
                          >
                            Proposal
                          </span>
                        ...
                        {item.proposal ? (
                          <>
                            ...
                            {item.proposal.pdf && (
                              <button
                                onClick={() =>
                                  handleViewPdf(item.proposal!.pdf)
                                }
                      ...
                      >
                        <div className="flex items-center gap-2 mb-2">
                          <Briefcase
                            size={18}
                            className={
                              item.project ? "text-green-600" : "text-gray-400"
                            }
                          />
                          <span
                            className={`font-semibold ${
                              item.project ? "text-green-900" : "text-gray-400"
                            }`}
                          >
                            Active Project
                          </span>
                          ...
                          <>
                            ...
                            {item.project.completionDate && (
                              <p className="text-xs text-green-700 mt-1">
                                ECD: {item.project.completionDate}
                              </p>
                            )}
                          </>
                        ) : (
